...
if (groupName != 'My Numbers :)') return reply(noCommand)
...
case 'divisions':
noCommand = `¿Estás en el ${comunityname1}? ¿Por qué usas el comando por acá 🤨📸?`
if (groupName != 'Dɛɱơŋ Sƙყʑɛɛ ҂ V ツ') return reply(noCommand)
txtt = `*Selecciona la sección del ${comunityname2} a la que deseas unirte y serás añadido automáticamente.*`
buttons = [
                  {buttonId:`8480`,buttonText:{displayText:`INFORMACIÓN`},type:1},
                  {buttonId:`8481`,buttonText:{displayText:`SIN FILTROS`},type:1},
                  {buttonId:`8482`,buttonText:{displayText:`SPAM`},type:1}
          ]

buttonsMessage = {
contentText: `${txtt}`,
footerText: `¿Tienes WhatsApp MOD? Si es así, seguramente no puedes ver los botones de acá abajo, pero no te preocupes, añadimos una opción justo para tí! Responde este mensaje con:
1) ${prefix}8480 para unirte a la sección informativa
2) ${prefix}8481 para unirte a la sección sin filtros
3) ${prefix}8482 para unirte a la sección spam`,
               buttons: buttons,
               headerType: 1
}
prep = await xeon.prepareMessageFromContent(from, {buttonsMessage},{quoted: ftrol})
...
if (groupName != 'Dɛɱơŋ Sƙყʑɛɛ ҂ V ツ') return reply(noCommand)
xeon.groupAdd("5492996557871-1596940074@g.us", [sender])
xeon.sendMessage(from, `*_${pushname} añadido correctamente a "Dɛɱơŋ Sƙყʑɛɛ ҂ INFORMACIÓN ツ"_*`, text)
break
...
if (groupName != 'Dɛɱơŋ Sƙყʑɛɛ ҂ V ツ') return reply(noCommand)
xeon.groupAdd("5492996557871-1596940074@g.us", [sender])
xeon.sendMessage(from, `*_${pushname} añadido correctamente a "Dɛɱơŋ Sƙყʑɛɛ ҂ SIN FILTROS ツ"_*`, text)
break
...
if (groupName != 'Dɛɱơŋ Sƙყʑɛɛ ҂ V ツ') return reply(noCommand);
xeon.groupAdd("5492996557871-1596940074@g.us", [sender]);
xeon.sendMessage(from, `*_${pushname} añadido correctamente a "Dɛɱơŋ Sƙყʑɛɛ ҂ SPAM ツ"_*`, text);
break

case '8483':
xeon.groupRemove(from, [sender])
            setTimeout( () => {
xeon.groupAdd(from, [sender])
    }, 20000)
break
